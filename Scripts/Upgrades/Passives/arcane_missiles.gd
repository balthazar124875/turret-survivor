...
func get_description() -> String:
	return "Magic damage has a [color=red]" + str(100 * (procChance * (1 + (player.luck * luckScaling)))) + "%[/color](" + str(procChance * 100) + "+" + str(procChance * luckScaling * 100) + "*" + IconHandler.get_icon_path("luck") + ") to fire a arcane missile."
