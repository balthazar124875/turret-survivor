...
func HitEnemy(body : Enemy):
	var poisonEffect = StatusEffect.new(GlobalEnums.ENEMY_STATUS_EFFECTS.POISONED, poison_duration, poison_damage_per_tick)
	var effects: Array[StatusEffect] = [poisonEffect]
